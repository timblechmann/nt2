...
NT2_TEST_CASE_TPL ( logical_not_integer__1_0,  BOOST_SIMD_SIMD_INTEGRAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(logical_not(boost::simd::One<vT>())&1!=0, boost::simd::Zero<sr_t>());
  NT2_TEST_EQUAL(logical_not(boost::simd::Zero<vT>())&1!=0, boost::simd::True<sr_t>());
...
NT2_TEST_CASE_TPL ( logical_not_real__1_0,  BOOST_SIMD_SIMD_REAL_TYPES)
{
  ...
  // specific values tests
  NT2_TEST_EQUAL(logical_not(boost::simd::Inf<vT>())&1!=0, boost::simd::Zero<sr_t>());
  NT2_TEST_EQUAL(logical_not(boost::simd::Minf<vT>())&1!=0, boost::simd::Zero<sr_t>());
  NT2_TEST_EQUAL(logical_not(boost::simd::Nan<vT>())&1!=0, boost::simd::Zero<sr_t>());
  NT2_TEST_EQUAL(logical_not(boost::simd::Zero<vT>())&1!=0, boost::simd::True<sr_t>());
