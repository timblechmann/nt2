...
namespace nt2
{
  namespace details
  {
				      ...
				      A0& y)
	{
	  ...
	  // 	bf::tie(x, e) = frexp(a0);
	  // 	std::cout << "x " << x << " e " << e << std::endl;
	  int_type x_lt_sqrthf = select(gt(single_constant<A0, 0x3f3504f3>(),x), Mone<int_type>(), Zero<int_type>());
	  e = e+x_lt_sqrthf;
	...
	static inline A0 log(const A0& a0)
	{
	  ...
	  return seladd(is_inf(y1),select( b_or(is_ltz(a0), is_nan(a0)), Nan<A0>(), y2),y1);
	...
	static inline A0 log2(const A0& a0)
	{
	  ...
	  A0 z1 = ((z+y)+x)+fe;
	  A0 y1 = a0-rec(abs(a0)); // trick to reduce selection testing
	  return seladd(is_inf(y1),select( b_or(is_ltz(a0), is_nan(a0)), Nan<A0>(), z1),y1);
	...
	static inline A0 log10(const A0& a0)
	{
	  ...
	  A0 y1 = a0-rec(abs(a0)); // trick to reduce selection testing
	  return seladd(is_inf(y1),select(b_or(is_ltz(a0), is_nan(a0)), Nan<A0>(), z),y1);
