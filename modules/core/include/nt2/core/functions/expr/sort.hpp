...
namespace nt2 { namespace ext
{
  ...
  private:
    ...
    //==============================================================================================
    BOOST_FORCEINLINE  void
    check(const char * mode, bool& up, size_t & dim, boost::mpl::long_<2> const &) const
    {
      BOOST_ASSERT_MSG( std::strcmp("descend", mode) == 0 || std::strcmp("ascend", mode) == 0,
                        "sorting direction must be 'ascend' or 'descend'"); 
      up = mode[0] == 'a';
    ...
    BOOST_FORCEINLINE  void
    check(const size_t d, bool& up, size_t & dim, boost::mpl::long_<2> const &) const
    {
      up = true;
      dim = d; 
    }
    ...
    void dosort(T & res,  bool up = true, size_t dim = 1) const
    {
      ...
      size_t h = nt2::height(res);
      if (h <= 1) return; 
      int nbcol =  numel(res)/h; //TODO write nslice
