...
namespace nt2 { namespace ext
{
  ...
  /////////////////////////////////////////////////////////////////////////////
  NT2_FUNCTOR_IMPLEMENTATION( nt2::tag::gamma_, boost::simd::tag::lrb_
            , (A0)
  ...
  if ((nb2 = nbtrue(lt12)) > 0)
    ...
    xden = xden * z +splat<A0>( g_q[i]);    
      ...
      for (int32_t i = 0; i < maximum(n); ++i)
        {
    bA0 t = bitwise_andnot(lt(splat<A0>(i), n), islt1);
    r *= sel(t, y, One<A0>());
    y = seladd(t, y, One<A0>()) ;
        }
      r =  sel(gt(y1, y), r1, r); 
  ...
  private :
    template < class AA0, class bAA0>
      static inline AA0 finalize(const AA0& a0, const AA0& res,  const AA0& fact, const bAA0& lezy)
      {
  bAA0 eqza0 = is_eqz(a0); 
  bAA0 integer =  bitwise_andnot(is_flint(a0), eqza0);
  return sel(eqza0, rec(a0), sel(lezy, ifnanelse(integer, fact/res), res));
      }
  }; 
} }
#endif
