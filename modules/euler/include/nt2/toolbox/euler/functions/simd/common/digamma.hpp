...
namespace nt2 { namespace ext
{
  ...
  if( (nb = nbtrue(test)) > 0)
    {
      ...
      remainder = ifnotelsezero (is_eqz(remainder), Pi<A0>()/tanpi(remainder));
      result = ifelsezero(test, remainder);   
  ...
  if((nb = nbtrue(test)))
    { // If we're above the lower-limit for the asymptotic expansion then use it:
      r1 = ifelsezero(test, digamma_imp_large(x, sA0()))+result;
  ...
  r2 = ifnotelsezero(test, digamma_imp_1_2(x, sA0()))+result;
  if (nb == 0) return r2;
